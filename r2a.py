...
class r2a:
    ...
    def __init__(self, options):
        ...
        if self.options.relaxed_parser:
            print "Parsing rules in relaxed mode !"
    ...
    def main(self):
        ...
        for snort_rule in self.rules:
            ...
            if not comments and not emptylines:
                try:
                    r = Rule(snort_rule,self.options.relaxed_parser)
...
def parseArgs():
    ...
    parser.add_option("-C", help="Read in suricata.yaml configuration file", action="store", type="string",
                      dest="suri_conf")
    parser.add_option("-B", help="Use a Suricata binary file from a custom location", action="store", type="string",
                      dest="suri_binary")
    parser.add_option("-L", help="Use a Suricata log directory from a custom location", action="store", type="string",
                      dest="suri_log")
    ...
    parser.add_option("-6", help="Enable IPv6 packet creation and testing for Suricata", action="store_true",
                      dest="IPv6")
    ...
    parser.add_option("-T", help="Test rule against current Suricata configuration", action="store_true",
                      dest="testSuricata")
    ...
    parser.add_option("-r", help="Relaxed rule parsing! Ignore unsupported keywords. This can lead to unexpected "
                                 "results", action="store_true", dest="relaxed_parser")
