...
class Rule:
    ignorable_keywords = ["classtype","metadata","isdataat"]
    raw = ''
    type = ''
    proto = ''
    rawsources = ''
    ...
    def __init__(self, rule,relaxed):
        ...
        if general != None and rawoptions != None:
            ...
            for i in optlist:
                ...
                if relaxed is not None and relaxed is True and k in self.ignorable_keywords:
                    print "Skipping property " + str(k)
                    continue

                else:
                    print "\nUnsupported keyword! \"" + str(k) + "\" Error parsing rule contents\n" + str(rule)
                    return 1
                    # break
